...
const iconByWorkType = {
  chapter_management: Layers,
  student_management: GraduationCap,
  lecture_management: BookOpen,
};

const ManagementListCard = ({ member, onDelete }) => {
  const Icon = iconByWorkType[member.work_type] || Users;
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm text-dark-text-secondary">
        <div>
          ...
          <span className="block text-white capitalize">
            {WORK_TYPE_LABELS[member.work_type] || member.work_type}
          </span>
        </div>
        <div>
...
const AddMemberDrawer = ({ open, onClose, onSubmit, loading }) => {
          ...
          >
            <div className="flex items-center justify-between px-6 py-6 border-b border-white/10">
              <div>
                ...
                <p className="text-sm text-dark-text-muted">Create access for a new management member</p>
              </div>
            ...
            <div className="space-y-2 text-sm text-dark-text-muted">
              {FILTER_OPTIONS.filter((option) => option.value !== 'all').map((option) => (
                <div key={option.value} className="flex items-center justify-between">
                  ...
                  <span className="text-white">{members.filter((m) => m.work_type === option.value).length}</span>
                </div>
              ))}
            </div>
          </div>
