...
const StudentPortalDashboard = () => {
  ...
  const refreshComments = useCallback(
    async (videoId, limit = 50) => {
      try {
        const response = await schoolPortalAPI.getVideoComments(videoId, limit);
        if (!response.status) {
  ...
  const handleSubmitComment = useCallback(
    async (event) => {
      ...
      try {
        ...
        setCommentDraft('');
        await refreshComments(selectedVideoId, comments.length + 5);
    ...
    [selectedVideoId, commentDraft, comments.length, refreshComments],
  );
  ...
  }, [selectedVideoId, videoDetail, activeNav, fetchSavedVideos]);
  ...
  const shareVideoToChat = useCallback(async () => {
    if (!videoDetail) {
      toast.error('Select a lecture before sharing.');
      return;
    }

    const peerEnrollment = selectedChatEnrollment;
    if (!peerEnrollment) {
      toast.error('Select a student in chat to share this video.');
      setActiveNav('Chat');
      return;
    }
    ...
    try {
      ...
      const response = await schoolPortalAPI.sendChatMessage(peerEnrollment, payload);
      if (!response.status) {
      ...
      setMessages((prev) => ({
        ...prev,
        [peerEnrollment]: updatedMessages,
      }));
      setActiveNav('Chat');
      ...
      >
        ...
        <div className="space-y-3 px-5 py-4 text-sm text-white/80">
          ...
          <div className="flex items-center justify-between text-xs text-white/40">
            ...
            <span className="inline-flex items-center gap-1">
              <Flame className="h-3.5 w-3.5" /> {(video.total_watch_count ?? 0).toLocaleString('en-IN')}
            </span>
          </div>
        </div>
      </article>
        ...
        <main className="flex flex-1 flex-col">
          {activeNav === 'Chat' ? (
                ...
                {savedLoading ? (
                  ...
                  <div className="grid gap-6 md:grid-cols-2 xl:grid-cols-3">
                    {savedVideos.map((video) => (
                      <article
                        key={video.id}
                        className={`group flex flex-col overflow-hidden rounded-[28px] border border-white/5 bg-[#141722] transition hover:border-primary-500/40 ${
                          video.id === selectedVideoId ? 'border-primary-500/50' : ''
                        }`}
                      >
                        <div className="relative">
                          <img
                          ...
                          <div className="absolute bottom-4 left-4 right-4 flex items-center justify-between">
                            <div className="max-w-[70%] space-y-1">
                              <p className="text-[10px] uppercase tracking-[0.4em] text-white/50">{video.subject || 'Lecture'}</p>
                              <h3 className="text-base font-semibold text-white line-clamp-2">{video.title || 'Untitled Lecture'}</h3>
                            </div>
                            <span className="rounded-full bg-white/10 px-3 py-1 text-[10px] font-semibold uppercase tracking-[0.2em] text-white/70">
                              {video.duration_seconds ? formatDurationLabel(video.duration_seconds) : '—'}
                            </span>
                          </div>
                        </div>

                        <div className="flex flex-1 flex-col justify-between space-y-4 px-5 py-5 text-sm text-white/80">
                          <div className="space-y-3 text-xs text-white/50">
                            <p className="flex items-center gap-2">
                              <Flame className="h-4 w-4" /> {(video.total_watch_count ?? 0).toLocaleString('en-IN')} views
                            </p>
                            <p className="flex items-center gap-2">
                              <Clock className="h-4 w-4" />{' '}
                              {video.user_watch_duration_seconds
                                ? `${Math.floor(video.user_watch_duration_seconds / 60)}:${String(video.user_watch_duration_seconds % 60).padStart(2, '0')}`
                                : 'Not started yet'}
                            </p>
                            <p className="flex items-center gap-2">
                              <Calendar className="h-4 w-4" /> Saved on{' '}
                              {video.user_last_watched_at
                                ? new Date(video.user_last_watched_at).toLocaleDateString()
                                : '—'}
                            </p>
                          </div>
                          ...
                          <div className="flex flex-wrap items-center gap-3 text-xs">
                            <button
                              ...
                              className="inline-flex flex-1 items-center justify-center gap-2 rounded-full bg-white px-4 py-2 font-semibold uppercase tracking-[0.2em] text-black transition hover:bg-primary-100"
                            >
                            ...
                            <button
                              type="button"
                              onClick={() => setSelectedVideoId(video.id)}
                              className={`inline-flex items-center justify-center gap-2 rounded-full px-4 py-2 font-semibold uppercase tracking-[0.2em] transition ${
                                video.user_subscribed ? 'bg-primary-500/20 text-primary-100 hover:bg-primary-500/30' : 'bg-white/10 text-white hover:bg-white/20'
                              }`}
                            >
                              <Bookmark className="h-4 w-4" />
                              {video.user_subscribed ? 'Saved' : 'Save'}
                            </button>
                          </div>
                        </div>
                      </article>
                  ...
                  <div className="space-y-6">
                    ...
                    <div className="space-y-4">
                      ...
                      {booksLoading ? (
                        ...
                        <div className="rounded-2xl border border-dashed border-white/10 bg-[#141722] px-6 py-16 text-center text-white/50">
            ...
            <>
                    ...
                    <div className="overflow-hidden rounded-[30px] border border-white/5 bg-[#141722] shadow-2xl">
                      <div className="relative aspect-video w-full bg-black/40">
                          ...
                          <React.Fragment>
                            ...
                            <div className="absolute bottom-6 left-6 right-6 flex flex-wrap items-center justify-between gap-4">
                              ...
                              <div className="flex flex-wrap items-center gap-3">
                                ...
                                >
                                  <Share2 className="h-4 w-4" /> {isSharingVideo ? 'Sharing…' : 'Share to Chat'}
                                </button>
                              </div>
                            </div>
                        </React.Fragment>
                        ...
                        <div className="absolute inset-0 flex flex-col items-center justify-center px-6 text-center text-sm text-red-400">
                          <p>{videoError}</p>
                          <button type="button" onClick={fetchDashboardVideos} className="mt-3 rounded-full bg-white/10 px-4 py-2 text-xs uppercase tracking-[0.2em] text-white transition hover:bg-white/20">Retry</button>
                        </div>
                      ) : (
                      ...
                      {videoDetail ? (
                        <div className="border-t border-white/5 bg-[#141722]/90 backdrop-blur px-6 py-6">
                          <div className="flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between">
                            <div className="grid gap-3 text-xs text-white/40 sm:grid-cols-3">
                              <div className="flex items-center gap-2">
                                <Clock className="h-4 w-4" /> Duration · {videoDetail.duration_seconds ? `${Math.floor(videoDetail.duration_seconds / 60)}:${String(videoDetail.duration_seconds % 60).padStart(2, '0')}` : '—'}
                              </div>
                              <div className="flex items-center gap-2">
                                <Flame className="h-4 w-4" /> Total Views · {(videoDetail.total_watch_count ?? 0).toLocaleString('en-IN')}
                              </div>
                              <div className="flex items-center gap-2">
                                <CheckCircle2 className="h-4 w-4" /> Your Watch Time · {videoDetail.user_watch_duration_seconds ? `${Math.floor(videoDetail.user_watch_duration_seconds / 60)}:${String(videoDetail.user_watch_duration_seconds % 60).padStart(2, '0')}` : '0:00'}
                              </div>
                            </div>
                            <div className="flex flex-wrap items-center gap-4">
                              <button
                                type="button"
                                onClick={() => recordVideoProgress(Math.floor(Math.random() * 180) + 60)}
                                className="inline-flex items-center gap-2 rounded-full bg-white px-5 py-2 text-xs font-semibold uppercase tracking-[0.2em] text-black transition hover:bg-primary-100"
                              >
                              ...
                              <button
                                ...
                                onClick={shareVideoToChat}
                                disabled={isSharingVideo}
                                className="inline-flex items-center gap-2 rounded-full bg-white/10 px-4 py-2 text-xs font-semibold uppercase tracking-[0.2em] text-white transition hover:bg-white/20 disabled:cursor-not-allowed disabled:opacity-60"
                              >
                                <Share2 className="h-4 w-4" /> {isSharingVideo ? 'Sharing…' : 'Share to Chat'}
                              </button>
                            </div>
                    ...
                    <section className="rounded-[28px] border border-white/5 bg-[#141722] p-6 shadow-lg">
                      <header className="flex items-center justify-between">
                        <div>
                          ...
                          <h3 className="mt-1 text-base font-semibold text-white">Related lectures</h3>
                        </div>
                    ...
                    <section className="rounded-[28px] border border-white/5 bg-[#141722] p-6 shadow-lg">
                      <header className="flex items-center justify-between">
                        <div>
                          ...
                          <h3 className="mt-1 text-base font-semibold text-white">Watched lectures</h3>
                        </div>
              ...
              >
                <div className="pointer-events-auto flex h-full w-full flex-col border-l border-white/10 bg-[#0F121D]">
                    ...
                    {comments.length ? (
                      comments.map((comment) => (
                        <div key={comment.id} className="rounded-2xl border border-white/5 bg-[#13172A] px-5 py-4">
                          <div className="flex items-center justify-between text-xs text-white/40">
                            <span>{comment.enrollment_number || 'Anonymous'}</span>
