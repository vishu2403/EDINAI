...
const ChapterDashboard = () => {
  ...
  const detectedAssistantPlan = useMemo(() => {
    const rawLabel = latestAssistantMeta?.planLabel || user?.package || '';
    return rawLabel ? rawLabel.toUpperCase() : '';
  }, [latestAssistantMeta, user]);
  const detectedAssistantPlanLimit = useMemo(
    () => (typeof latestAssistantMeta?.planLimit === 'number' ? latestAssistantMeta.planLimit : null),
    [latestAssistantMeta]
  );
  ...
  const renderLectureOverview = () => {
    return (
      <motion.div className="card p-6 space-y-6" initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }}>
        <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <p className="text-sm uppercase tracking-wide text-dark-text-muted">Lecture Overview</p>
            <h2 className="text-2xl font-bold text-dark-text-primary">Chapter Management</h2>
            <p className="text-sm text-dark-text-muted">{lectureOverview.total} lecture{lectureOverview.total === 1 ? '' : 's'} generated so far.</p>
          </div>
          <div className="flex items-center gap-2">
            <Button variant="secondary" size="small" icon={RefreshCw} onClick={loadLectureOverview} loading={lectureOverviewLoading}>
              Refresh
            </Button>
          </div>
        </div>

        {lectureOverviewLoading ? (
          <div className="flex min-h-[160px] items-center justify-center text-dark-text-muted">
            <Loader2 className="h-5 w-5 animate-spin" />
            <span className="ml-2 text-sm">Loading lecture overview…</span>
          </div>
        ) : lectureOverviewError ? (
          <div className="rounded-xl border border-red-500/40 bg-red-500/5 p-4 text-sm text-red-200">
            {lectureOverviewError}
          </div>
        ) : hasLectureOverviewItems ? (
          <div className="grid gap-4 md:grid-cols-2">
            {lectureOverview.items.map((lecture) => renderLectureCard(lecture))}
          </div>
        ) : (
          <div className="rounded-xl border border-dashed border-dark-border/80 bg-dark-card/40 p-8 text-center">
            ...
            <p className="text-sm text-dark-text-muted">No lectures generated yet. Start by uploading a chapter PDF and extracting topics.</p>
          </div>
        )}
      </motion.div>
    );
      ...
      try {
        ...
        if (!cancelled && response.status) {
          ...
          const filtered = materials.filter((material) => {
            const materialSubject = material.subject?.toLowerCase() || '';
            return materialSubject.includes(subjectQuery);
          });
          setSuggestedMaterials(filtered);
        }
      } catch (error) {
        if (!cancelled) {
  ...
  const handleSkipContext = () => {
    ...
    setTopicGenerationMode(false);
    toast.info('Skipping suggestions. Upload your chapter PDF directly.');
  };
  ...
  const handleGenerateTopics = async (
    ...
    try {
      ...
      if (response.status) {
        const topicsMap = response.data?.topics || {};
        ...
        materialIds.forEach(matId => {
          ...
          const matchingKey = Object.prototype.hasOwnProperty.call(topicsMap, normalizedKey)
            ? normalizedKey
            : matId;
          const materialTopics = topicsMap[matchingKey];
          const topicsList = materialTopics?.topics || [];
          const materialIdNumber = Number(matId);
          const detectedChapterTitle = (materialTopics?.chapter_title || '').trim();
              ...
              <div className="space-y-3">
                <div className="grid gap-3 sm:grid-cols-2">
                  ...
                  <div className="space-y-1">
                    ...
                    <div className="rounded-md border border-primary-500/40 bg-dark-card/50 px-3 py-2 text-sm font-semibold text-primary-100">
                      {detectedAssistantPlan || 'Auto-detecting…'}
                    </div>
                    <p className="text-[11px] text-primary-200/80">
                      {detectedAssistantPlanLimit
                        ? `Plan allows up to ${detectedAssistantPlanLimit} assistant topics.`
                        : 'Limit will adjust automatically once detected.'}
                    </p>
                  </div>
                </div>
                <Button
