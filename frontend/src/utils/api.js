...
api.interceptors.request.use(
  (config) => {
    const token = localStorage.getItem('inai_token');
    if (token) {
...
export const chapterMaterialsAPI = {
  ...
  listMaterials: async (filters = {}) => {
    const response = await api.get('/chapter-materials/', { params: filters });
    return response.data;
  },
...
export const lectureAPI = {
  getConfig: async ({ language, duration } = {}) => {
    ...
    const response = await api.get('/chapter-materials/lecture/config', { params });
    return response.data;
  },
  ...
  listGeneratedLectures: async (filters = {}) => {
    ...
    const response = await api.get('/chapter-materials/lectures', { params });
    return response.data;
  },

  getLectureOverview: async (filters = {}) => {
    const response = await api.get('/chapter-materials/lectures', { params: filters });
    return response.data;
  },
  ...
  generateLecture: async ({
    ...
    const payload = {
      material_id: materialId,
      selected_topic_indices: selectedTopicIndices,
    };
    ...
    const response = await api.post('/chapter-materials/lecture/generate', payload, {
  ...
  chat: async ({ lectureId, question, answerType, isEditCommand = false, contextOverride }) => {
    const response = await api.post(
      `/chapter-materials/lecture/${lectureId}/chat`,
      {
        question,
...
export const schoolPortalAPI = {
  ...
  sendChatMessage: async (peerEnrollment, message) => {
    const response = await api.post(
      '/school-portal/chat/messages',
      { peer_enrollment: peerEnrollment, message },
      {
        headers: {
          ...getStudentPortalAuthHeaders(),
        },
  ...
  getVideoComments: async (videoId, limit = 50) => {
