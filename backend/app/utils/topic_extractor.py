...
def extract_text_with_auto_language(pdf_path: Path) -> Tuple[str, Optional[str]]:
    """Run OCR, detect the dominant language from the content, then ensure OCR uses that language."""

    def _run_ocr(_: str) -> str:
        try:
            return read_pdf_with_ocrmypdf(pdf_path, ocr_language="eng+hin+guj")
        except RuntimeError as exc:  # pragma: no cover - external dependency
            logger.warning("OCR failed for multi-language OCR: %s", exc)
            return ""
    ...
    if not ocr_text.strip():
        logger.info("OCR attempts did not produce text for %s; falling back to embedded PDF text", pdf_path.name)
    ...
    logger.info("Final Detected Language: %s", detected_language)
...
def extract_topics_from_pdf(pdf_path: Path) -> Dict[str, Any]:
    ...
    if not language_code:
        logger.error(
            "Language detection failed for %s; skipping topic extraction.",
            pdf_path.name,
        )
        return {
            "success": False,
            ...
            "error": "Unable to detect language from the PDF; topics were not generated.",
        }
