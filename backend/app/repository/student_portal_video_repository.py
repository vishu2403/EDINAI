...
def ensure_sample_videos(admin_id: int, std: Optional[str], samples: Iterable[Dict[str, Any]]) -> None:
    ...
    with get_pg_cursor() as cur:
        ...
        count = cur.fetchone()["total"] if cur.rowcount else 0
...
def list_comments(video_id: int, *, limit: int = 50) -> List[Dict[str, Any]]:
    query = """
        SELECT id, video_id, enrollment_number, comment, created_at, like_count
        FROM student_portal_video_comments
    ...
    with get_pg_cursor() as cur:
        cur.execute(query, {"video_id": video_id, "limit": limit})
        return cur.fetchall()
