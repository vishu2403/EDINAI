...
def _ensure_chat_message_columns() -> None:
    """Ensure chat message table has attachment-related columns."""
...
def get_roster_entry(enrollment_number: str) -> Optional[Dict[str, Any]]:
    query = (
        ...
        "FROM student_roster_entries WHERE LOWER(enrollment_number) = LOWER(%(enrollment_number)s) LIMIT 1"
    )
...
def get_student_roster_context(enrollment_number: str) -> Optional[Dict[str, Any]]:
    query = """
        ...
        WHERE LOWER(r.enrollment_number) = LOWER(%(enrollment_number)s)
        LIMIT 1
    """
