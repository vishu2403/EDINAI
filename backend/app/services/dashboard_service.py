...
def get_admin_dashboard(admin_id: int) -> Dict[str, object]:
    ...
    plan_label = (admin.get("package") or "").strip().lower() or None
    ...
    days_until_expiry = (
        (admin["expiry_date"] - datetime.utcnow()).days if admin.get("expiry_date") else 0
    )
    ...
    return {
        ...
        "account_status": {
            ...
            "expiry_date": admin["expiry_date"].isoformat() if admin.get("expiry_date") else None,
