...
def change_password(request: ChangePasswordRequest, current_user: Dict[str, object]) -> None:
    ...
    if current_user["role"] == "admin":
        auth_repository.update_admin_password(user_obj["admin_id"], hashed)
    else:
...
def forgot_password(request: ForgotPasswordRequest) -> Tuple[str, Dict[str, object]]:
    ...
    return "Reset code generated (demo mode)", data
...
def reset_password(request: ResetPasswordRequest) -> None:
    ...
    if not (reset_code.isdigit() and len(reset_code) == 6):
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Invalid reset code format")
    ...
    if admin:
        auth_repository.update_admin_password(admin["admin_id"], hashed, updated_at=datetime.utcnow())
    if member:
