...
def _prepare_generation_from_merged(
    *,
    ...
    if primary_material:
        ...
        subject_value = (primary_material.get("subject") if isinstance(primary_material, dict) else primary_material.subject) or "subject"
    else:
        std_value = "merged"
    ...
    if primary_material:
        metadata.update(
            {
                "primary_material_id": primary_material.get("id") if isinstance(primary_material, dict) else primary_material.id,
                "primary_material_subject": primary_material.get("subject") if isinstance(primary_material, dict) else primary_material.subject,
                "primary_material_std": primary_material.get("std") if isinstance(primary_material, dict) else primary_material.std,
            }
        )

    log_context = {
        "material_id": (primary_material.get("id") if isinstance(primary_material, dict) else primary_material.id) if primary_material else None,
        "std": (primary_material.get("std") if isinstance(primary_material, dict) else primary_material.std) if primary_material else "Merged",
        "subject": (primary_material.get("subject") if isinstance(primary_material, dict) else primary_material.subject) if primary_material else "Merged Topics",
        "board": (primary_material.get("board") if isinstance(primary_material, dict) else primary_material.board) if primary_material else "Mixed",
        "sem": (primary_material.get("sem") if isinstance(primary_material, dict) else primary_material.sem) if primary_material else "Mixed",
    ...
    material_snapshot = {
        "id": (primary_material.get("id") if isinstance(primary_material, dict) else primary_material.id) if primary_material else None,
        "admin_id": (primary_material.get("admin_id") if isinstance(primary_material, dict) else primary_material.admin_id) if primary_material else creator_admin_id,
        "std": (primary_material.get("std") if isinstance(primary_material, dict) else primary_material.std) if primary_material else log_context["std"],
        "subject": (primary_material.get("subject") if isinstance(primary_material, dict) else primary_material.subject) if primary_material else log_context["subject"],
        "board": (primary_material.get("board") if isinstance(primary_material, dict) else primary_material.board) if primary_material else log_context["board"],
        "sem": (primary_material.get("sem") if isinstance(primary_material, dict) else primary_material.sem) if primary_material else log_context["sem"],
        "chapter_number": (primary_material.get("chapter_number") if isinstance(primary_material, dict) else getattr(primary_material, "chapter_number", None)) if primary_material else None,
        "file_name": (primary_material.get("file_name") if isinstance(primary_material, dict) else getattr(primary_material, "file_name", None)) if primary_material else None,
    }
    ...
    try:
        ...
        for material_id in material_ids:
            ...
            try:
                ...
                # Permission check
                if current_user["role"] == "admin":
                    if (material.get("admin_id") if isinstance(material, dict) else material.admin_id) != current_user["id"]:
                    ...
                    member_admin_id = _resolve_member_admin_id(current_user)
                    if (material.get("admin_id") if isinstance(material, dict) else material.admin_id) != member_admin_id:
                        logger.warning(f"Access denied for material {material_id}")
                        topics_by_material.append(entry)
                        continue
    ...
    if not audio_path.exists():
        ...
        try:
            ...
            with get_pg_cursor() as cur:
                cur.execute("SELECT lecture_data FROM lecture_gen WHERE lecture_uid = %(lecture_uid)s", {"lecture_uid": lecture_id})
                row = cur.fetchone()
            ...
            # Extract slide number from filename (e.g., "slide-8.mp3" -> 8)
            try:
            ...
            # Find the slide with matching number
            target_slide = None
            for slide in slides:
            ...
            return FileResponse(audio_path_result, media_type="audio/mpeg")
