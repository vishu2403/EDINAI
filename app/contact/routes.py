...
async def upsert_contact(
    ...
    if contact:
        ...
        if image_paths:
            updates["image_path"] = ";".join(
                _serialize_paths(contact.image_path) + _serialize_paths(image_paths)
            )
        if center_photo_paths:
            updates["center_photos"] = ";".join(
                _serialize_paths(contact.center_photos) + _serialize_paths(center_photo_paths)
            )
        if logo_paths:
            updates["logo_path"] = ";".join(
                _serialize_paths(contact.logo_path) + _serialize_paths(logo_paths)
            )
        if activity_paths:
            updates["other_activities_path"] = ";".join(
                _serialize_paths(contact.other_activities_path) + _serialize_paths(activity_paths)
            )
